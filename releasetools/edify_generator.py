...
class EdifyGenerator(object):
  ...
  def AssertBuildType(self, buildType):
    """Assert that the device identifier is the given string."""
    cmd = ('getprop("ro.build.type") == "%s" || '
           'abort("E%d: This package is for \\"%s\\" devices; '
           'this is a \\"" + getprop("ro.build.type") + "\\".");') % (
               buildType,common.ErrorCode.BUILDTYPE_MISMATCH,buildType)
    self.script.append(cmd)
  ...
  def AddToZip(self, input_zip, output_zip, input_path=None):
    ...
    if input_path is None:
      print "use OTA/bin/updater "
      data = input_zip.read("OTA/bin/updater")
    else:
      print "not use OTA/bin/updater "
