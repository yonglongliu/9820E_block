...
def SignOutput(temp_zip_name, output_zip_name):
 ...
 # pw = key_passwords[OPTIONS.package_key]
  ...
  common.SignFile(temp_zip_name, output_zip_name, OPTIONS.package_key, "XR_B191_888",
                  whole_file=True)
...
# @}
  ...
  updating_logo = True
  imgname = os.path.join(OPTIONS.target_tmp,"IMAGES/logo.img")
  if os.path.exists(imgname):
    target_logo = common.File(imgname, open(imgname).read())
    imgname = os.path.join(OPTIONS.source_tmp,"IMAGES/logo.img")
    if os.path.exists(imgname):
      source_logo = common.File(imgname, open(imgname).read())
      include_full_logo = False
    else:
      updating_logo = False
      include_full_logo = True
  else:
    updating_logo = False
    include_full_logo = False

  if updating_logo:
    updating_logo = (source_logo.data != target_logo.data)
  ...
  build_type = GetBuildProp("ro.build.type", OPTIONS.target_info_dict)
  script.AssertBuildType(build_type)
...
""" % bcb_dev)
  ...
  if updating_logo:
    logo_type, logo_device = common.GetTypeAndDevice(
        "/logo", OPTIONS.source_info_dict)
    d = common.Difference(target_logo, source_logo)
    _, _, d = d.ComputePatch()
    if d is None:
      include_full_logo = True
      common.ZipWriteStr(output_zip, "logo.img", target_logo.data)
    else:
      include_full_logo = False

      print "logo      target: %d  source: %d  diff: %d" % (
        target_logo.size, source_logo.size, len(d))

      common.ZipWriteStr(output_zip, "patch/logo.img.p", d)
      script.PatchCheck("%s:%s:%d:%s:%d:%s" %
                      (logo_type, logo_device,
                       source_logo.size, source_logo.sha1,
                       target_logo.size, target_logo.sha1))
  else:
    if include_full_logo:
      common.ZipWriteStr(output_zip, "logo.img", target_logo.data)
...
""" % bcb_dev)
  ...
  if updating_logo:
    if include_full_logo:
      script.Print("Installing logo image...")
      ...
      script.ApplyPatch("%s:%s:%d:%s:%d:%s"
                        ...
                        "-",
                        target_logo.size, target_logo.sha1,
                        source_logo.sha1, "patch/logo.img.p")
      print "logo image changed; including."
  else:
    if include_full_logo:
      script.Print("Installing logo image...")
      script.WriteRawImage("/logo","logo.img")
    else:
      print "logo image unchanged; skipping."
...
#add build type check
  buildType = GetBuildProp("ro.build.type", OPTIONS.target_info_dict)
  script.AssertBuildType(buildType)
  # Dump fingerprints
...
def main(argv):
  ...
  args = common.ParseOptions(argv, __doc__,
                             extra_opts="b:k:r:i:d:wne:t:a:2o:HCSR:sysimg",
                             extra_long_opts=[
                                 "board_config=",
                                 "package_key=",
                                 "incremental_from=",
